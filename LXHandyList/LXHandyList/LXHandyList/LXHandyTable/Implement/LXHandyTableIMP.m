...
@implementation LXHandyTableIMP
#pragma mark - UITableViewDelegate

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    LXHTableSection *htSection = self.sectionArray[indexPath.section];
    id<LXHTableCellConfig> config = htSection.rowArray[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    LXHTableSection *htSection = self.sectionArray[section];
    id<LXHTableHeaderFooterConfig> config = htSection.header;
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    LXHTableSection *htSection = self.sectionArray[section];
    id<LXHTableHeaderFooterConfig> config = htSection.footer;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell conformsToProtocol:@protocol(LXHTableCellProtocol)]) {
        UITableViewCell<LXHTableCellProtocol> *tmpCell = (UITableViewCell<LXHTableCellProtocol> *)cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell respondsToSelector:@selector(ybht_didSelectedAtIndexPath:)]) {
        [(id<LXHTableCellProtocol>)cell ybht_didSelectedAtIndexPath:indexPath];
    }
}

#pragma mark - private

- (Class)validClassForCellConfig:(id<LXHTableCellConfig>)config {
    return config.ybht_cellClass ?: UITableViewCell.self;
}

- (Class)validClassForHeaderFooterConfig:(id<LXHTableHeaderFooterConfig>)config {
    return config.ybht_headerFooterClass ?: UIView.self;
}

- (NSString *)reuseIdentifierForCellConfig:(id<LXHTableCellConfig>)config {
    NSString *identifier;
...
- (NSString *)reuseIdentifierForHeaderFooterConfig:(id<LXHTableHeaderFooterConfig>)config {
    NSString *identifier;
...
- (CGFloat)heightForHeaderFooterWithTableView:(UITableView *)tableView config:(id<LXHTableHeaderFooterConfig>)config section:(NSInteger)section {
...
- (__kindof UIView *)viewForHeaderFooterWithTableView:(UITableView *)tableView config:(id<LXHTableHeaderFooterConfig>)config section:(NSInteger)section {
    ...
    if ([view conformsToProtocol:@protocol(LXHTableHeaderFooterProtocol)]) {
        UIView<LXHTableHeaderFooterProtocol> *tmpView = (UIView<LXHTableHeaderFooterProtocol> *)view;
...
- (NSMutableArray<LXHTableSection *> *)sectionArray {
    if (!_sectionArray) {
...
- (LXHTCommonInfo *)commonInfo {
    if (!_commonInfo) {
        _commonInfo = [LXHTCommonInfo new];
    }
    return _commonInfo;
}
